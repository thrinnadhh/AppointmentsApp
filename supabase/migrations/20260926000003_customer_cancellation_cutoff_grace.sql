-- =============================================================================
-- Migration: 20260926000003_customer_cancellation_cutoff_grace.sql
-- Description:
--   Update customer cancellation cutoff in cancel_booking() and
--   protect_booking_payment_fields() so that cancellations at 45 minutes
--   before slot start (> 30m) receive a 100% full refund.
-- =============================================================================
...
-- ─────────────────────────────────────────────────────────────────────────────
-- UPDATE protect_booking_payment_fields TRIGGER
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF NEW.payment_status IS DISTINCT FROM OLD.payment_status THEN
    ...
    -- If client attempts status change on cancellation, enforce 30-minute window (> 30m = full refund)
    v_minutes_to_slot := EXTRACT(EPOCH FROM (OLD.slot_start - NOW())) / 60;
    IF v_minutes_to_slot > 30 AND NEW.payment_status NOT IN ('REFUND_PENDING', 'REFUNDED') THEN
