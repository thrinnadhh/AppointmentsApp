-- =============================================================================
-- Migration: 20260925000001_refund_pending_status_and_60m_window.sql
-- Description:
--   1. Extend payment_status ENUM with 'REFUND_PENDING' and 'REFUND_FAILED'.
--   2. Update cancel_booking() and record_no_show() to set payment_status = 'REFUND_PENDING'
--      as an intermediate status prior to external payment gateway settlement.
--   3. Reconcile cancellation window: Enforce 60-minute cutoff (> 60m before slot = full
--      deposit refund; <= 60m = late cancellation, deposit forfeited).
-- =============================================================================
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. RECONCILE cancel_booking() RPC
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF v_is_merchant THEN
    -- Merchant cancellations ALWAYS fully refund customer
    v_eligible := true;
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 3. RECONCILE record_no_show() RPC
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    -- Strike 1 or 2: Courtesy Grace Period -> Full deposit refund (REFUND_PENDING)
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 4. UPDATE protect_booking_payment_fields TRIGGER (60-minute cutoff & refund transitions)
-- ─────────────────────────────────────────────────────────────────────────────
...
DECLARE
  v_minutes_to_slot NUMERIC;
BEGIN
  -- Allow service_role or internal trusted writes
  ...
  IF NEW.payment_status IS DISTINCT FROM OLD.payment_status THEN
    ...
    -- If client attempts status change on cancellation, enforce 60-minute window
    v_minutes_to_slot := EXTRACT(EPOCH FROM (OLD.slot_start - NOW())) / 60;
    IF v_minutes_to_slot > 60 AND NEW.payment_status NOT IN ('REFUND_PENDING', 'REFUNDED') THEN
      RAISE EXCEPTION 'Cancellations more than 1 hour before the slot must be refunded, not %', NEW.payment_status;
    ELSIF v_minutes_to_slot <= 60 AND NEW.payment_status != 'FORFEITED' THEN
